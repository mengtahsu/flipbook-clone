...
Returns ranked, filtered image results.
"""
import json
...
# Preferred free/CC image sources
_PREFERRED = {
...
def _score(img: dict) -> float:
    ...
    # Prefer landscape-ish aspect ratio
    if w > 0 and h > 0:
        ratio = w / h
        if 1.2 <= ratio <= 2.0:
            score += 2
    ...
    # Longer title = more context
    if len(img.get("title", "")) > 30:
        score += 0.5
